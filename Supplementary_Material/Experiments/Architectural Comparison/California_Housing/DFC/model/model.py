...
class DualTransformer(nn.Module):
    ...
    def forward(self, cat_data, num_data, est_data):
        ...
        # === Combine ===
        ori_embedding = torch.cat([ori_cat_emb, ori_num_emb], dim=1).squeeze(2)
        tar_embedding = torch.cat([tar_cat_emb, tar_num_emb], dim=1).squeeze(2)
        all_embedding = torch.cat([tar_embedding, ori_embedding], dim=1)
        
        # === Two-Stage Attention ===
        # fea_final_out   = self.FEAWAL(all_embedding).mean(1)
        # enc_final_out   = self.ENCWAL(all_embedding).mean(1)
        fea_final_out = self.NORWAL(all_embedding)
