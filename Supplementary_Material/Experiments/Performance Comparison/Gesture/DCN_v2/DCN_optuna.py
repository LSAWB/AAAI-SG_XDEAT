...
def objective(trial):
    ...
    # === Quantile Transformer for Numerical Features ===
    ...
    x_train_num = quantile_transformer.transform(train_data[NUM_COLS]).astype(np.float32)
    x_valid_num = quantile_transformer.transform(valid_data[NUM_COLS]).astype(np.float32)
    x_test_num  = quantile_transformer.transform(test_data[NUM_COLS]).astype(np.float32)
